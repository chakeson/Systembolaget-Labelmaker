import re, getopt, sys, urllib.request, string, timeit
...
def input_validation(list_of_pages):
    ...
    pattern_regex_website = re.compile(r'^(https:\/\/www.systembolaget.se/produkt/|http:\/\/www.systembolaget.se/produkt/|https:\/\/www.systembolaget.se/\d*)')
    ...
    for iterator in range(len(list_of_pages)):
        ...
        if result[0] != "":
            list_of_pages[iterator] = "https://www.systembolaget.se/"+str(list_of_pages[iterator])
...
def extract_drink_data(page_html):
    ...
    try:
        #Gets the country
        pattern_wine_location1 = re.compile(r"\"country\"\:\"[a-zA-Z\s\S]*\",\"originLevel2")
    ...
    try:
        ...
        wine_alc_procentage = "Failed to fetch wine alcohole procentage. Regex 7 failure. "
    except Exception as e:
    ...
    try:
        ...
        pattern_wine_suger_content = re.compile(r'\"sugarContentGramPer100ml\"\:\"\s*\d*\S*\d*\"\,\"additives\"\:')
    ...
    try:
        #Gets the taste and usage recommendations
        pattern_usage = re.compile(r'\"usage\"\:\"[a-zA-Z\s\S]*\"\,\"alcoholPercentage\"')
...
def create_labels(class_list_wines):
    ...
    for wine in class_list_wines:
        ...
        new_file.write(wine.price +"kr "+ "sysnr:" + wine.productnr +"\t" + wine.alcohol_procentage + "% " + wine.suger_amount +"g/100ml \t" + str(datum) +"\n")
